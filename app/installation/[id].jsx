...
export default function InstallationDetailScreen() {
  ...
  const getInstallations = async () => {
    try {
      const response = await claimsApi.get(`installations/details/${id}`);
  ...
  const closeInstallation = async () => {
    try {
      ...
      const usedMaterialsData = usedMaterials.map(({ id_material, quantity,price }) => ({ id_material, quantity,price }));
  
      const payload = {
        news:observations,
        id_installation: installationData.id_installation,
        id_service: installationData.Service.id_service,
        id_mobile: installationData.id_mobile,
        id_client: installationData.Service.id_client,
        photos: photoData,
        usedMaterialsData,
      ...
      console.log(usedMaterialsData);
  
      // Envía la solicitud POST al backend
      const response = await claimsApi.post('/installations/close', payload);
  ...
  const serviceLocation = installationData?.Service?.Location;
    ...
    <SafeAreaView>
      <ScrollView>
        <View style={styles.container}>
          <Text variant='titleLarge'>Detalles de la instalacion {id}</Text>
        </View>

        <Details data={installationData} />
        <Divider />
        <View>
        ...
        <Description description={installationData.observations} installation={true}  news={installationData.news} />

        <Divider />

        <View>
          <AssignedMaterials service_type={installationData.Service.Service_type.description} />
        </View>

        <Divider />

        <Observations text={observations} setText={setObservations} installation={true} />
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginBottom: 20 }}>
          <Button mode='contained' onPress={closeInstallation}>
            Cerrar reclamo
          </Button>
